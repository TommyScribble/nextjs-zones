...
import useDropdownMenu, { DropdownMenuOptions } from './useDropdown'
...
const TestComponent: React.FC<Props> = ({ options }) => {
    ...
    const {
        ...
        setIsOpen,
        moveFocus,
    } = useDropdownMenu(itemCount, options)

    const clickHandlers: (() => void)[] = [
        (): void => dummyFunction('Item one clicked'),
    ...
    return (
        <React.Fragment>
            ...
            <button
                id="remove-item"
                onClick={(): void => setItemCount((prevCount) => prevCount - 1)}
            >
                Remove Item
            </button>
            ...
            <button
                data-testid="focus-third-item"
                onClick={(): void => moveFocus(2)}
            >
                Focus third item
            </button>

            <span data-testid="is-open-indicator">
                {isOpen ? 'true' : 'false'}
            </span>
        </React.Fragment>
    )
...
it('Activates the click handler of a menu item after pressing enter while focused on it', () => {
    ...
    expect(dummyFunction).toHaveBeenCalledWith('Item one clicked')
})
